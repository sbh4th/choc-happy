...
## Adjusted model
linear.1 <- lm(happy ~ as.factor(treated) + as.factor(period), data=choc)
# stargazer
stargazer(linear.1, notes="")
# texreg
texreg::texreg(list(linear.1), custom.note="%stars. texreg")
## Product model
## linear.2 <- lm(happy ~ as.factor(treated) * as.factor(period), data=choc)
## Table of results
## t2 <- stargazer(linear.1, linear.2, title="Effect", align=TRUE, type='html')
## kable(t2)
```
